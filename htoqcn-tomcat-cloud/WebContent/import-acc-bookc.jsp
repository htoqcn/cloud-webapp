...
<html>
	<head>
		<title>Import Account Book</title><%-- SELECT month, sum(amount) from (SELECT SUBSTR(thedate, 1, 7) as month, amount from account_book where flaga is null) group by month --%>
	</head>
	<body>
<%
...
%>
		Import Steps:<br>
		1) Access /month-bill-query and copy to this textarea<br>
		2) Update flaga = 1 for necessary items<br>
		3) /import-account-book-ext.jsp
		4) Check MainID of ACCOUNT_BOOK_EXT
		<form method="post">
			<table>
				<tr>
					<td>
...
<%
	if (content != null) {
		...
		try {
			...
			pstmt = conn.prepareStatement("INSERT INTO ACCOUNT_BOOKC (AddTime, Amount, TheDate, Description, PayDate, PayWay, Flag) VALUES (?, ?, ?, ?, ?, ?, ?)");
			boolean error = false;
